...
//
// File:         FreeRTOS TCP/IP Demo with Light Snake Feature and Reverse Direction
//
// **************************************************************************
//
...
#define TASK_NAME_LIGHT_SNAKE "light_snake"
...
QueueHandle_t snake_direction_queue;
...
void task_light_snake(void *tp_arg);
...
bool read_input_leds(int8_t* input_msg) {
    char led_str[] = "LED ";
    int i = 0;

    while (led_str[i] != '\0') {
        if (input_msg[i] == '\0' || input_msg[i] != led_str[i]) {
            return false;
        }
        i++;
    }
    ...
    for( ;; )
    {
        ...
        for ( l_reply_count = 0; pdTRUE; l_reply_count++ )
        {
            ...
            if( l_len > 0 )
            {
                ...
                PRINTF("Parsed LED Position: %d\n", position);

                if(read_input_leds(l_rx_buf)) {
                    if(position >=0 && position < LED_PTC_NUM){
...
void task_light_snake(void *tp_arg) {
    ...
    BaseType_t recv_result;

    while (1) {

        recv_result = xQueueReceive(snake_direction_queue, &direction, portMAX_DELAY);
        if (recv_result == pdPASS) {
            if (direction == FORWARD) {
                PRINTF("Starting Light Snake Forward Sequence.\r\n");
                ...
                PRINTF("Light Snake Forward Sequence Completed.\r\n");
            }
            else if (direction == BACKWARD) {
                PRINTF("Starting Light Snake Backward Sequence.\r\n");
                ...
                PRINTF("Light Snake Backward Sequence Completed.\r\n");
            }
            else {
                PRINTF("Received Unknown Direction Command: %d\r\n", direction);
            }
        }
        else {
            PRINTF("Failed to receive direction command from queue.\r\n");
        }
    }
}
...
int main(void) {
    ...
    PRINTF("FreeRTOS+TCP with Light Snake and Reverse Direction started.\r\n");
    ...
    snake_direction_queue = xQueueCreate(SNAKE_QUEUE_LENGTH, SNAKE_QUEUE_ITEM_SIZE);
    if (snake_direction_queue == NULL) {
        PRINTF("Failed to create snake direction queue.\r\n");
    }


    if (xTaskCreate(
            task_light_snake,
            TASK_NAME_LIGHT_SNAKE,
