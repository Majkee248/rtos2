...
//
// File:         FreeRTOS TCP/IP Demo with Enhanced LED Control
//
// **************************************************************************
//
...
// #define TASK_NAME_SOCKET_CLI    "socket_cli" // Optional, commented out
...
// #define TASK_NAME_LIGHT_SNAKE "light_snake" // Removed

#define LED_COMMAND_QUEUE_LENGTH 10
#define LED_COMMAND_QUEUE_ITEM_SIZE sizeof(struct LedCommand)
...
// Removed SnakeDirection_t and related queue
// typedef enum {
//     FORWARD = 0,
//     BACKWARD
// } SnakeDirection_t;

// Removed SNAKE_QUEUE_LENGTH and SNAKE_QUEUE_ITEM_SIZE
// #define SNAKE_QUEUE_LENGTH 10
// #define SNAKE_QUEUE_ITEM_SIZE sizeof(SnakeDirection_t)
...
// Array for PTB LEDs (Right)
CUSTOM_LED ptb_bool[ LED_PTB_NUM ] =
        {
                { false, LED_PTB2_PIN, LED_PTB2_GPIO },
                { false, LED_PTB3_PIN, LED_PTB3_GPIO },
                { false, LED_PTB9_PIN, LED_PTB9_GPIO },
                { false, LED_PTB10_PIN, LED_PTB10_GPIO },
                { false, LED_PTB11_PIN, LED_PTB11_GPIO },
                { false, LED_PTB18_PIN, LED_PTB18_GPIO },
                { false, LED_PTB19_PIN, LED_PTB19_GPIO },
                { false, LED_PTB20_PIN, LED_PTB20_GPIO },
                { false, LED_PTB23_PIN, LED_PTB23_GPIO },
        };
...
// Function to parse LED commands
bool parse_led_command(const char* str, LedCommand_t* cmd) {
    if (strncmp(str, "LED ", 4) != 0) return false;

    // Ensure there is enough length
    if (strlen(str) < 6) return false;

    // Find 'L' or 'R'
    char side_char = str[4];
    if (side_char != 'L' && side_char != 'R') return false;
    cmd->side = (side_char == 'L') ? SIDE_LEFT : SIDE_RIGHT;

    // Find the number after 'L' or 'R'
    int num;
    if (sscanf(str + 5, "%d", &num) !=1) return false;
    if (num <1) return false;
    cmd->number = (uint8_t)num;

    return true;
}

// Task to handle socket server
void task_socket_srv( void *tp_arg )
{
    ...
    for( ;; )
    {
        ...
        for ( l_reply_count = 0; pdTRUE; l_reply_count++ )
        {
            ...
            if( l_len > 0 )
            {
                ...
                // Parse and handle LED commands
                if (strncmp((char*)l_rx_buf, "LED ", 4) == 0 ) {
                    LedCommand_t cmd;
                    if (parse_led_command((char*)l_rx_buf, &cmd)) {
                        if (xQueueSend(led_command_queue, &cmd, 0) != pdPASS) {
                            PRINTF("Failed to send LED command to queue.\r\n");
                        }
                        else {
                            PRINTF("LED command received: side=%s, number=%d\n",
                                   (cmd.side == SIDE_LEFT) ? "L" : "R", cmd.number);
                        }
                    }
                    else {
                        PRINTF("Invalid LED command format.\n");
                    }
                }
        ...
        // Pause to prevent network congestion
        vTaskDelay( SOCKET_SRV_TOUT / portTICK_PERIOD_MS );
...
// Task for the socket client (optional, commented out in this example).
/*
...
// Task to set LEDs based on their state
void task_set_onoff( void *tp_arg ){
    while(1) {
        // Update PTC LEDs
        ...
        // Update PTB LEDs
        for(int i = 0; i < LED_PTB_NUM; i++) {
            GPIO_PinWrite( ptb_bool[ i ].gpio, ptb_bool[ i ].pin, ptb_bool[i].state ? 1 : 0 );
        }
    ...
    while (1) {
        for (int i = 0; i < BUT_NUM; i++) {
            ...
            if (but_bool[i].state != prev_state) {
                ...
                // Removed snake direction commands
                /*
                if ((i == 0 || i == 1) && but_bool[i].state) {
                    if (i == 0) {
                        PRINTF("PTC9 Button Pressed. Triggering Light Snake Forward.\r\n");
                        SnakeDirection_t direction = FORWARD;

                        if (xQueueSend(snake_direction_queue, &direction, 0) != pdPASS) {
                            PRINTF("Failed to send FORWARD command to snake queue.\r\n");
                        }
                    }
                    else if (i == 1) {
                        PRINTF("PTC10 Button Pressed. Triggering Light Snake Backward.\r\n");
                        SnakeDirection_t direction = BACKWARD;

                        if (xQueueSend(snake_direction_queue, &direction, 0) != pdPASS) {
                            PRINTF("Failed to send BACKWARD command to snake queue.\r\n");
                        }
                    }
                }
                */
...
// Task to print button states and send to client if changed
void task_print_buttons(void *tp_arg) {
    bool enter = false;
    char msg[64];
...
// New Task to handle LED sequencing based on commands
void task_led_sequence(void *tp_arg) {
    LedCommand_t cmd;
    while (1) {
        if (xQueueReceive(led_command_queue, &cmd, portMAX_DELAY) == pdPASS) {
            if (cmd.side == SIDE_LEFT) {
                PRINTF("Starting LED Sequence from Left: %d LEDs.\r\n", cmd.number);
                for (int i = 0; i < cmd.number && i < LED_PTC_NUM; i++) {
                ...
                PRINTF("Starting LED Sequence from Right: %d LEDs.\r\n", cmd.number);
                for (int i = 0; i < cmd.number && i < LED_PTB_NUM; i++) {
                    ptb_bool[i].state = true;
                    PRINTF("LED PTB%d ON.\r\n", i);
                    vTaskDelay(200 / portTICK_PERIOD_MS);
                }
            }
            else {
                PRINTF("Received Unknown Side Command: %d\r\n", cmd.side);
            }
        }
        else {
            PRINTF("Failed to receive LED command from queue.\r\n");
        }
    }
}
...
int main(void) {
    ...
    PRINTF("FreeRTOS+TCP with Enhanced LED Control started.\r\n");
    ...
    led_command_queue = xQueueCreate(LED_COMMAND_QUEUE_LENGTH, LED_COMMAND_QUEUE_ITEM_SIZE);
    if (led_command_queue == NULL) {
        PRINTF("Failed to create LED command queue.\r\n");
    }

    // Create LED Sequence Task
    if (xTaskCreate(
            task_led_sequence,
            "led_sequence",
