...
bool parse_led_command(const char* input, Direction_t* dir, int* num) {
    int i = 0;
    while (input[i] == ' ') i++;
    if (strncmp(&input[i], "LED", 3) != 0) return false;
    i += 3;
    while (input[i] == ' ') i++;
                ...
                if(parse_led_command((char*)l_rx_buf, &direction, &num_leds)) {
                    if(num_leds >= 0 && num_leds <= LED_PTC_NUM) {
                        ...
                        if(xQueueSend(blink_command_queue, &cmd, 0) != pdPASS) {
                            ...
                            PRINTF("Blink command received: direction=%s, num_leds=%d\n", (direction == LEFT) ? "LEFT" : "RIGHT", num_leds);
                        }
                    } else {
...
void vApplicationIPNetworkEventHook(eIPCallbackEvent_t t_network_event) {
    ...
    if(t_network_event == eNetworkUp) {
        ...
        if(s_task_already_created == pdFALSE) {
            if(xTaskCreate(task_socket_srv, TASK_NAME_SOCKET_SRV, configMINIMAL_STACK_SIZE + 1024, (void*)SOCKET_SRV_PORT, configMAX_PRIORITIES - 1, NULL) != pdPASS) {
