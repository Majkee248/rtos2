...
#define SOCKET_SRV_BUF_SIZE        512
    ...
    for( ;; )
    {
        ...
        for (l_reply_count = 0; pdTRUE; l_reply_count++)
        {
            ...
            if( l_len > 0 )
            {
                ...
                int num_leds;
                if (strncmp(l_rx_buf, LIST_TASKS_COMMAND, strlen(LIST_TASKS_COMMAND)) == 0) {
                    char task_list_buffer[SOCKET_SRV_BUF_SIZE];
                    get_task_list(task_list_buffer, sizeof(task_list_buffer));
                    FreeRTOS_send(l_sock_client, (void *)task_list_buffer, strlen(task_list_buffer), 0);
                }
                else if (parse_led_command((char *)l_rx_buf, &direction, &num_leds)) {
...
void task_print_buttons(void *tp_arg) {
    ...
    while (true) {
        ...
        if (enter) {
            if(l_sock_client != FREERTOS_INVALID_SOCKET){
                FreeRTOS_send(l_sock_client, (void *)msg_buffer, strlen(msg_buffer) + 1, 0);
            }
            enter = false;
        }
        vTaskDelay(1 / portTICK_PERIOD_MS);
